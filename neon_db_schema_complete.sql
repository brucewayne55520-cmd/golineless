-- ============================================================
-- Go LineLess — Complete Database Schema (PostgreSQL / Neon)
...
-- ============================================================
-- 5. RUNNERS
-- Comrade (delivery partner) accounts. Includes KYC fields,
...
CREATE TABLE "tasks" (
  ...
  -- Comrade Dispatch: Pickup & Live Proof
  ...
  -- Smart Dispatch Metrics (Phase 3)
...
CREATE INDEX "idx_tasks_runner_id"            ON "tasks" USING btree ("runner_id");
...
CREATE INDEX "idx_tasks_status_runner"        ON "tasks" USING btree ("status", "runner_id");
CREATE INDEX "idx_tasks_status_category"      ON "tasks" USING btree ("status", "category");
CREATE INDEX "idx_tasks_user_status"          ON "tasks" USING btree ("user_id", "status");
CREATE INDEX "idx_tasks_runner_status"        ON "tasks" USING btree ("runner_id", "status");
CREATE INDEX "idx_tasks_completed_at"         ON "tasks" USING btree ("completed_at");
CREATE INDEX "idx_tasks_payment_method_status" ON "tasks" USING btree ("payment_method", "payment_status");
...
-- ============================================================
-- 11. SUBSCRIPTIONS
-- Active customer subscriptions. Links user → plan.
-- ============================================================
...
CREATE TABLE "admin_settings" (
  ...
  -- Waiting Revenue V2 Brackets (cumulative)
  ...
  -- Queue ETA Multipliers (Phase 9.3)
...
-- ============================================================
-- 13. DEVICE TOKENS
-- Push notification device tokens (FCM-ready).
-- ============================================================
...
-- ============================================================
-- 14. RUNNER LOCATIONS
-- GPS breadcrumb trail for each runner during task execution.
-- ============================================================
...
-- ============================================================
-- 15. RECRUITMENTS
-- Candidate pipeline for new comrade recruitment.
-- Stages: applied → interview_scheduled → documents_submitted →
--         training_pending → training_complete → pilot_active → suspended
-- ============================================================
...
-- ============================================================
-- 16. TRAINING MODULES
-- Comrade onboarding curriculum items.
-- ============================================================
...
-- ============================================================
-- 17. RUNNER TRAINING
-- Per-comrade training completion records.
-- ============================================================
...
-- ============================================================
-- 18. QUALITY REVIEWS
-- Post-task quality scoring and SLA grading.
-- ============================================================
...
-- ============================================================
-- 19. INCIDENTS
-- Track complaints, misconduct, GPS failures, payment issues.
-- ============================================================
...
-- ============================================================
-- 20. SUPPORT TICKETS
-- Customer and comrade support requests.
-- ============================================================
...
-- ============================================================
-- 21. FRAUD FLAGS
-- Individual fraud/risk detection events.
-- Types: invalid_status_transition, gps_validation_failed,
...
CREATE INDEX "idx_fraud_flag_task_id"    ON "fraud_flags" USING btree ("task_id");
CREATE INDEX "idx_fraud_flag_runner_id"  ON "fraud_flags" USING btree ("runner_id");
CREATE INDEX "idx_fraud_flag_type"       ON "fraud_flags" USING btree ("type");
CREATE INDEX "idx_fraud_flag_created_at" ON "fraud_flags" USING btree ("created_at");


-- ============================================================
-- 22. PROOF PHOTOS
-- Individual proof photo records with GPS and metadata.
-- Proof types: reached_pickup, reached_task_location,
--              in_progress, completed, general
-- ============================================================
...
CREATE INDEX "idx_proof_photo_task_id"       ON "proof_photos" USING btree ("task_id");
CREATE INDEX "idx_proof_photo_runner_id"     ON "proof_photos" USING btree ("runner_id");
CREATE INDEX "idx_proof_photo_type"          ON "proof_photos" USING btree ("proof_type");
CREATE INDEX "idx_proof_photo_created_at"    ON "proof_photos" USING btree ("created_at");


-- ============================================================
-- 23. TASK TIMELINE EVENTS
-- Structured audit trail for task lifecycle events.
-- Types: pending, assigned, on_the_way, reached_pickup,
--        reached_task_location, in_progress, completed,
--        cancelled, proof_photo, cash_pending, cash_confirmed,
--        cash_disputed, refunded, otp_verified, payment_pending,
--        queue_updated, waiting_started, waiting_paused,
--        waiting_completed
-- ============================================================
...
CREATE INDEX "idx_timeline_task_id"         ON "task_timeline_events" USING btree ("task_id");
CREATE INDEX "idx_timeline_status"          ON "task_timeline_events" USING btree ("status");
CREATE INDEX "idx_timeline_event_timestamp" ON "task_timeline_events" USING btree ("event_timestamp");


-- ============================================================
-- 24. RUNNER PAYOUTS
-- Settlement records for comrade earnings.
...
CREATE INDEX "idx_payaudit_task_id"    ON "payment_audit_log" USING btree ("task_id");
CREATE INDEX "idx_payaudit_created_at" ON "payment_audit_log" USING btree ("created_at");


-- ============================================================
-- 26. VERIFICATION SESSIONS
-- Photo verification challenge sessions for fraud prevention.
-- Challenge IDs format: VER-YYYY-XXXXX
-- ============================================================
...
CREATE INDEX "idx_vsession_user_id"     ON "verification_sessions" USING btree ("user_id");
CREATE INDEX "idx_vsession_task_id"     ON "verification_sessions" USING btree ("task_id");
CREATE INDEX "idx_vsession_status"      ON "verification_sessions" USING btree ("status");
CREATE INDEX "idx_vsession_challenge_id" ON "verification_sessions" USING btree ("challenge_id");


-- ============================================================
-- 27. PHOTO UPLOADS
-- Individual photo records with hash, watermark, and risk data.
-- ============================================================
...
CREATE INDEX "idx_photo_session_id"      ON "photo_uploads" USING btree ("session_id");
CREATE INDEX "idx_photo_task_id"         ON "photo_uploads" USING btree ("task_id");
CREATE INDEX "idx_photo_runner_id"       ON "photo_uploads" USING btree ("runner_id");
CREATE INDEX "idx_photo_original_hash"   ON "photo_uploads" USING btree ("original_hash");
CREATE INDEX "idx_photo_verification_id" ON "photo_uploads" USING btree ("verification_id");
CREATE INDEX "idx_photo_status"          ON "photo_uploads" USING btree ("status");
CREATE INDEX "idx_photo_created_at"      ON "photo_uploads" USING btree ("created_at");
...
CREATE INDEX "idx_vaudit_session_id"   ON "verification_audit_logs" USING btree ("session_id");
CREATE INDEX "idx_vaudit_photo_id"     ON "verification_audit_logs" USING btree ("photo_id");
CREATE INDEX "idx_vaudit_action"       ON "verification_audit_logs" USING btree ("action");
CREATE INDEX "idx_vaudit_created_at"   ON "verification_audit_logs" USING btree ("created_at");


-- ============================================================
-- 29. VERIFICATION HASHES
-- Deduplication index for original image files.
-- ============================================================
...
CREATE INDEX "idx_vhash_original_hash" ON "verification_hashes" USING btree ("original_hash");
...
ALTER TABLE "runner_sessions"
  ADD CONSTRAINT "runner_sessions_runner_id_runners_id_fk"
...
ALTER TABLE "tasks"
  ADD CONSTRAINT "tasks_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");
...
ALTER TABLE "notifications"
  ADD CONSTRAINT "notifications_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");
...
-- Reviews
ALTER TABLE "reviews"
  ADD CONSTRAINT "reviews_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");
...
ALTER TABLE "reviews"
  ADD CONSTRAINT "reviews_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");
...
ALTER TABLE "device_tokens"
  ADD CONSTRAINT "device_tokens_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

-- Runner locations
ALTER TABLE "runner_locations"
  ADD CONSTRAINT "runner_locations_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

ALTER TABLE "runner_locations"
  ADD CONSTRAINT "runner_locations_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

-- Runner training
ALTER TABLE "runner_training"
  ADD CONSTRAINT "runner_training_module_id_training_modules_id_fk"
  FOREIGN KEY ("module_id") REFERENCES "training_modules"("id");

-- Quality reviews
ALTER TABLE "quality_reviews"
  ADD CONSTRAINT "quality_reviews_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

ALTER TABLE "quality_reviews"
  ADD CONSTRAINT "quality_reviews_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

-- Incidents
ALTER TABLE "incidents"
  ADD CONSTRAINT "incidents_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

ALTER TABLE "incidents"
  ADD CONSTRAINT "incidents_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

ALTER TABLE "incidents"
  ADD CONSTRAINT "incidents_user_id_users_id_fk"
  FOREIGN KEY ("user_id") REFERENCES "users"("id");

-- Support tickets
ALTER TABLE "support_tickets"
  ADD CONSTRAINT "support_tickets_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

-- Fraud flags
ALTER TABLE "fraud_flags"
  ADD CONSTRAINT "fraud_flags_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

ALTER TABLE "fraud_flags"
  ADD CONSTRAINT "fraud_flags_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

-- Proof photos
ALTER TABLE "proof_photos"
  ADD CONSTRAINT "proof_photos_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

ALTER TABLE "proof_photos"
  ADD CONSTRAINT "proof_photos_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

-- Task timeline events
ALTER TABLE "task_timeline_events"
  ADD CONSTRAINT "task_timeline_events_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

-- Runner payouts
ALTER TABLE "runner_payouts"
  ADD CONSTRAINT "runner_payouts_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

-- Payment audit log
ALTER TABLE "payment_audit_log"
  ADD CONSTRAINT "payment_audit_log_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

-- Verification sessions
ALTER TABLE "verification_sessions"
  ADD CONSTRAINT "verification_sessions_user_id_users_id_fk"
  FOREIGN KEY ("user_id") REFERENCES "users"("id");

ALTER TABLE "verification_sessions"
  ADD CONSTRAINT "verification_sessions_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

-- Photo uploads
ALTER TABLE "photo_uploads"
  ADD CONSTRAINT "photo_uploads_session_id_verification_sessions_id_fk"
  FOREIGN KEY ("session_id") REFERENCES "verification_sessions"("id");

ALTER TABLE "photo_uploads"
  ADD CONSTRAINT "photo_uploads_task_id_tasks_id_fk"
  FOREIGN KEY ("task_id") REFERENCES "tasks"("id");

ALTER TABLE "photo_uploads"
  ADD CONSTRAINT "photo_uploads_user_id_users_id_fk"
  FOREIGN KEY ("user_id") REFERENCES "users"("id");

ALTER TABLE "photo_uploads"
  ADD CONSTRAINT "photo_uploads_runner_id_runners_id_fk"
  FOREIGN KEY ("runner_id") REFERENCES "runners"("id");

-- Verification audit logs
ALTER TABLE "verification_audit_logs"
  ADD CONSTRAINT "verification_audit_logs_session_id_verification_sessions_id_fk"
  FOREIGN KEY ("session_id") REFERENCES "verification_sessions"("id");

ALTER TABLE "verification_audit_logs"
  ADD CONSTRAINT "verification_audit_logs_photo_id_photo_uploads_id_fk"
  FOREIGN KEY ("photo_id") REFERENCES "photo_uploads"("id");

-- Verification hashes
ALTER TABLE "verification_hashes"
  ADD CONSTRAINT "verification_hashes_photo_id_photo_uploads_id_fk"
  FOREIGN KEY ("photo_id") REFERENCES "photo_uploads"("id");
...
-- ============================================================
-- SCHEMA COMPLETE
-- 29 tables | 40+ indexes | 40+ foreign keys | 3 seed rows
-- ============================================================
