    ...
    <div className="flex min-h-screen gl-surface dark:bg-[#0A0E1A]">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-5">
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">City Heatmap</h1>
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-48 border border-gray-200 dark:border-[#374151] rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-[#1F2937] dark:text-gray-100 gl-transition"
            />
            <button onClick={addCustomArea} className="p-1.5 rounded-xl bg-white dark:bg-[#1F2937] border border-gray-200 dark:border-[#374151] hover:bg-gray-50 dark:hover:bg-[#111827] gl-transition" title="Add area">
              <Plus size={14} className="text-gray-900 dark:text-blue-600" />
            </button>
          </div>
        </div>
        ...
        {data && (
          <div className="grid grid-cols-3 gap-4 mb-5">
            <div className="bg-white dark:bg-[#111827] rounded-2xl p-4 gl-shadow-md border border-gray-200 dark:border-[#1F2937] text-center gl-transition">
              <MapPin size={18} className="mx-auto mb-1 text-gray-900 dark:text-blue-600" />
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{data.totalDemand}</p>
          ...
          {(data?.areas || []).map((area: Required<import("@workspace/api-client-react").HeatmapDataAreasItem>, i: number) => {
            ...
            return (
              <motion.div
                ...
                className={`bg-white dark:bg-[#111827] rounded-2xl p-5 gl-shadow-md border ${shortage ? "border-[#FECACA] dark:border-[#7F1D1D]" : "border-gray-200 dark:border-[#1F2937]"} hover:gl-shadow-lg gl-transition`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <MapPin size={14} style={{ color: shortage ? "#EF4444" : DARK }} />
                    <h3 className="font-bold text-gray-900 dark:text-gray-100">{area.area}</h3>
                    {isCustom && (
                  ...
                  <div>
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                      <motion.div
                        ...
                        className="h-full rounded-full" style={{ background: "#3B82F6", width: `${demandPercent}%` }}
                      />
                    </div>
