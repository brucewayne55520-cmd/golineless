    ...
    <div className="flex min-h-screen gl-surface dark:bg-[#0A0E1A]">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Recruitment Pipeline</h1>
            <p className="text-gray-400 text-xs mt-0.5">{total} total recruits</p>
          </div>
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-gray-900 text-sm font-bold gl-shadow-md gl-transition hover:gl-shadow-lg active:scale-[0.98]" style={{ background: BLUE_GRAD }}>
            <Plus size={16} /> Add Recruit
          </button>
        </div>

        {/* Funnel */}
        <div className="bg-white dark:bg-[#111827] rounded-2xl p-5 gl-shadow-md border border-gray-200 dark:border-[#1F2937] mb-5">
          <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm mb-3">Recruitment Funnel</h3>
        ...
        <div className="bg-white dark:bg-[#111827] rounded-2xl gl-shadow-md border border-gray-200 dark:border-[#1F2937] overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-200 dark:border-[#1F2937]">
            <h3 className="font-bold text-gray-900 dark:text-gray-100">All Recruits</h3>
          </div>
          {loading ? (
            ...
            <div className="divide-y divide-gray-100 dark:divide-[#1F2937]">
              {(Array.isArray(recruits) ? recruits : []).map((r: Required<import("@workspace/api-client-react").Recruit>) => (
                <motion.div key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-[#1F2937] gl-transition">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-gray-900 text-sm font-bold flex-shrink-0" style={{ background: BLUE_GRAD }}>
                    {r.name?.[0] || "?"}
                  </div>
        ...
        {showAdd && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowAdd(false)}>
            <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
              <h3 className="font-black text-gray-900 mb-4">Add New Recruit</h3>
              <div className="space-y-3">
                {["name", "phone", "area"].map(f => (
                  <input key={f} value={form[f as keyof typeof form]} onChange={e => setForm({ ...form, [f]: e.target.value })}
                    placeholder={f.charAt(0).toUpperCase() + f.slice(1)} className="w-full border border-gray-200 dark:border-[#374151] rounded-xl px-3 py-2.5 text-sm bg-white dark:bg-[#1F2937] dark:text-gray-100 gl-transition" />
                ))}
                <select value={form.vehicleType} onChange={e => setForm({ ...form, vehicleType: e.target.value })}
                  className="w-full border border-gray-200 dark:border-[#374151] rounded-xl px-3 py-2.5 text-sm bg-white dark:bg-[#1F2937] dark:text-gray-100 gl-transition">
                ...
                <input value={form.languages} onChange={e => setForm({ ...form, languages: e.target.value })}
                  placeholder="Languages (comma-separated)" className="w-full border border-gray-200 dark:border-[#374151] rounded-xl px-3 py-2.5 text-sm bg-white dark:bg-[#1F2937] dark:text-gray-100 gl-transition" />
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={() => setShowAdd(false)} className="flex-1 py-3 rounded-xl border border-gray-200 dark:border-[#374151] text-sm font-semibold gl-transition">Cancel</button>
                <button onClick={addRecruit} className="flex-1 py-3 rounded-xl text-gray-900 text-sm font-bold gl-transition" style={{ background: BLUE_GRAD }}>Add Recruit</button>
              </div>
            </div>
          </div>
        )}
