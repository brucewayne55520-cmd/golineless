...
export async function applyServerWatermark(params: WatermarkParams): Promise<WatermarkResult> {
  ...
  const svgOverlay = `
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Gold accent line -->
      <rect x="0" y="${height - barHeight}" width="${width}" height="3" fill="#C9A84C" />

      <!-- Line 1: Brand + Task + Type -->
      <text x="${padding}" y="${height - barHeight + fontSize + padding}" font-family="monospace" font-size="${fontSize}" fill="#C9A84C" font-weight="bold">
        ${escapeXml(line1)}
      </text>
      ...
      <text x="${padding}" y="${height - barHeight + (fontSize * 3) + padding + 4}" font-family="monospace" font-size="${Math.round(fontSize * 0.85)}" fill="#C9A84C">
        ${escapeXml(line3)}
      </text>
